...
| Поиск                | Ranker, Searcher, filter, workers     | Сложный  | 740          | 74.00 — 185.00          | 7.400 — 18.500        | 1.12 МБ/с     |
...
| Итого                |                                       |          | **269199**   | **1,292.59 — 3,231.48** | **125.887 — 314.717** | **399.4 МБ/с**|  
...
$$\text{CPU} = 1293 \times 2.5 = 3232 \text{ cores}$$  
$$\text{RAM} = 126 \times 2.5 = 315 \text{ GB}$$  


Для того, чтобы обработать такие нагрузки были выбраны следующая конфигурация сервера:
* CPU: AMD EPYC 9454P - 48 ядер; 96 потоков; 2.75-3.8 GHz; 256 MB L3 cache - 4598 USD
* RAM: 4x32 GB DDR5 Kingston Fury 5.6 GHz - 941 USD

${3232 \text{cores}}/{48 \text{cores}} \approx 68$ - сервера для обеспечения процессорной мощности с резервом  
${315 \text{GB}}/{128 \text{GB}} \approx 3$ - сервера для обеспечения ОЗУ мощности с резервом

Как итог для обепечения необходимых мощностей с учетом резерва было взято 68 серверов с вышеперечисленной конфигураций с приблизительной стоимостью за штуку 6000 USD.  
Общая стоимость развертывания - 408.000$  
Амортизация за 5 лет: 6.800$/месяц  
...
Конфигурация схожая с вышеописанной - m5.8xlarge, которая также предоставляет 48 ядер и 128 гб ОЗУ<br>
Стоимость - 0.18432$/час. Или 720*0.18432=132.71$/мес<br>
Итого требуется 68 * 132.71$ = **9024.28$/мес**
...
Стоимость HDD на 8 TB ~200$

Итого: <br>
$$2929 \text{PB}\times{0.0021 \text{ dollars/(GB*мес)}} = 64.496.861 \text{ dollars/мес}$$ - размещение на AWS S3  

7322 PB / 8 TB * 200 dollars = 187.456.000 dollars - размещение на собственном оборудовании  
Амортизация: 187.456.000/60 = 3.124.267 dollars 
