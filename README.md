...
| Поставить лайк/дизлайк | $2.584\approx3$                      |
| Подписаться на канал   | $0.076\approx0$                      |
| Выложить ролик         | $0.152\approx0$                      |
| Написать комментарий   | $0.532\approx1$                      |
| Добавить в избранное   | $0.4256\approx0$                     |
| Поделиться             | $0.152\approx0$                      |
| Поиск                  | $0.73\approx1$                       |
| Технология Duet/Stitch | $0.0289\approx0$                     |
...
|------------------|------------------|-------------------|---------------|------------------|
| Азия             | 36%              | 293.843           | 7             | 41978 (5.14%)    |
| Северная Америка | 12%              | 97.948            | 3             | 32649 (4%)       |
| Южная Америка    | 10%              | 81.624            | 2             | 40812 (4.99%)    |
| Европа           | 10%              | 81.624            | 2             | 40812 (4.99%)    |
| Африка           | 11%              | 89.786            | 2             | 44893 (5.5%)     |
...
**Инфраструктура балансировки: _Kubernetes_**, так как использовать только bare metal - дорого и непростая конфигурация. Также Kubernetes имеет ряд преимуществ перед просто VM. Среди них:  
	1. Кэширование  
	2. Архивация  
	3. SSL termination  
	4. API Gateway  
	5. Persistence/sticky sessions  


Типы балансировки с уровнем OSI/ISO и преимуществами/недостатками:


| Тип балансировки                  | Подходит для сервиса | Комментарий |
|-----------------------------------|----------------------|-----------------------------|
| DNS                               | Нет                  | Имеет ряд недостатков, таких как: кэширование на прикладном уровне, высокая нагрузка, проблема с продолжительными tcp соединения                    |
| L4                                | Да                   | Старый, но проверенный. Имеет уязвимости, однако выделяется наивысшей эффективностью. Из плюсов: максимальная прозрачность и производительность                       |
| BGP Anycast                       | Нет                  | Довольно тяжелая конфигурация и настройка топологии. Преимущество в виде повышенной отказоустойчивости можно возместить failover/self-healing                    |
| L7                                | Да                   | Требовательна к системе и имеет невысокую производительность. Однако можно выделить: высокую гипкость, надежность, равномерное распределение, решение проблем медленных клиентов                       |
| Redirect based load balancing     | Нет                  | Несмотря на повышенную отказоустойчивость имеет нецелесообразное замедление latency                 |
| Application based load balancing  | Частично             | В зависимости от типа трафика можно распределить нагрузку в соответствии с приоритетом                     |
| Client based load balancing       | Нет                  | Не подходит в силу политики компании                       |
| Функциональное распределение      | Нет                  | Сложность администрирования и возможность попасть на неделимую часть сайта          |  


&nbsp;&nbsp;&nbsp;&nbsp;В качестве **L4** баласировщика был выбран **Direct Routing**. Он является наиболее производительным, так как NAT имеет завышенную нагрузку на CPU и повторно проходящий трафик через балансер. В то время, как ip tunneling имеет существенный недостаток в сложной настройке инкапсуляции и повышенной нагрузки на процессор балансировщика.


### Отказоустойчивость


&nbsp;&nbsp;&nbsp;&nbsp;Для решения проблемы с преждевременным закрытием соединения воспользуемся методом квантилей. Если соединение простаивает больше чем **99.99 квантиль**, то оно закрывается.  


&nbsp;&nbsp;&nbsp;&nbsp;В случае если один из серверов упал и нужно перераспределить нагрузку - можно воспользоваться политикой **failover**. Первостепенно он подразумевает проверку работоспособности (ping, port, get, cgi-bin) и выполнение алгоритма выбора другого бекенда в случае отказа текущего в процессе обработки запроса. Из основных алгоритмов являются:  
	1. Active-Passive  
	_2. Active-Active_  
	3. Cold-Warm-Hot  


&nbsp;&nbsp;&nbsp;&nbsp;Для быстрого восстановления после падения nginx можно воспользоваться keep-alive балансировкой соединений.  
	Для уровня **L3: VRRP** (Virtual Router Redundancy Protocol)  
	Для уровня **L7: Service discovery Kubernetes**  


### SSL-терминация
&nbsp;&nbsp;&nbsp;&nbsp;Для расшифровки HTTPS трафика на прокси-сервере воспользуемся SSL-терминацией. Существует 2 типа возобновления SSL-сессий: session id и session ticket. Первый вариант устаревший и редко используется. Поэтому воспользуемся session ticket, который работает на TLS 1.3. Известно[^16], что в среднем время соединения TLS-handshake занимает около 117 мс. Рассчитаем максимальную нагрузку при пиковом RPS в одном ДЦ.  


**Пиковый RPS на 1 ДЦ**:

$$
\mathrm{RPS}_{\text{peak1}} = \mathrm{RPS}_{\text{avg}}\times 0.05 \times 2 = 0.10 \times \mathrm{RPS}_{\text{avg}} \approx 81623
$$

**Вычислительное время**:

$$
\mathrm{RPS}_{\text{peak1}} \times \text{TLS handshake} = 81623 \times 117\ \text{ms} \approx 9549.9\ \text{сек}
$$
