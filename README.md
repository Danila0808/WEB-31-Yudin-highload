...
**Choreography Saga** - набор локальных транзакций. Каждая локальная транзакция обновляет базу данных и публикует сообщение или событие, инициируя следующую локальную транзакцию в саге. Если транзакция завершилась неудачей, например, из-за нарушения бизнес правил, тогда сага запускает компенсирующие транзакции, которые откатывают изменения, сделанные предшествующими локальными транзакциями. В отличие от saga с оркестратором (где есть центральный координатор, управляющий потоком), хореография опирается на событийно-ориентированную архитектуру. Вместо того чтобы один сервис приказывал другим, что делать, каждый микросервис:​
* Выполняет свою локальную транзакцию
* Публикует событие о том, что произошло
* Слушает события от других сервисов
* Реагирует на эти события независимо
...
* Использование уровня надежности **Tier IV** (максимальная отказоустойчивость)
	+ Полное дублирование всей инфраструктуры по схеме 2(N+1)
 	+ Полная физическая разнесенность критических систем по разным помещениям
  	+ Как минимум 4 независимых источника питания от разных сетей
...
|------------|-------------|-------------|
| Простая    | 0.1-0.2     | 50-150 MB   |
| Средняя    | 0.5-1       | 200-500 MB  |
| Сложная    | 2-5         | 500-2000 MB |

|  MVP функционал      | Сервисы                               | Характер | RPS на 1 ДЦ  | CPU (cores)   | RAM (GB)      | Трафик     |
|----------------------|---------------------------------------|----------|--------------|---------------|---------------|------------|
| Просмотр ролика      | Video, Workers, Syncs, Event Worker   | Средний  | 77032        | 39-78         | 15.04-37.6    | 122.8 МБ/с |
| Просмотр профиля     | User, Syncs                           | Простой  | 4055         | 0.4-0.8       | 0.195-0.586   | 2.46 МБ/с  |
| Публикация ролика    | Upload Service                        | Простой  | 27           | 0.0027-0.0054 | 0.001-0.003   | 42 КБ/с    |
| Лайк/дизлайк         | Video Reaction, Workers, Event Worker | Простой  | 2619         | 0.2619-0.5238 | 0.128-0.384   | 154 КБ/с   |
| Комментарий          | Comment, Syncs                        | Средний  | 540          | 2-3           | 0.512-1.28    | 335.4 КБ/с |
| Подписка             | Subscription, Syncs, Event Worker     | Простой  | 77           | 0.0077-0.154  | 0.004-0.012   | 4.2 КБ/с   |
| Регистрация          | Auth Service                          | Простой  | 3            | 0.0003-0.0006 | 0.0001-0.0003 | 2.2 КБ/с   |
| Поделиться           | Share Service, Syncs, Event woker     | Простой  | 154          | 0.0154-0.0308 | 0.0075-0.0225 | 8.4 КБ/с   |
| Добавить в избранное | Favourite Service, Syncs, Event worker| Простой  | 431          | 0.0431-0.0862 | 0.021-0.063   | 23.5 КБ/с  |
| Поиск                | Ranker, Searcher, filter, workers     | Сложный  | 740          | 2-3           | 0.36-1.08     | 1.12 МБ/с  |
| Recommend system     | Ranker, Recommender, filter, workers  | Сложный  | 4055         | 9-21          | 1.98-7.9      | 6.18 МБ/с  |
| API Gateway          | API Gateway                           | Простой  | 89733        | 9-18          | 4.38-13.14    | 133.13 МБ/с|
| Envoy                | Envoy                                 | Простой  | 89733        | 9-18          | 4.38-13.14    | 133.13 МБ/с|
| Итого                |                                       |          | **269199**       | **62-183**       | **26.63-75.19**    | **399.4 МБ/с**|  
...
${183 \text{cores}}/{48 \text{cores}} \approx 4$ - 4+1 сервера для обеспечения процессорной мощности с резервом  
${75.19 \text{GB}}/{128 \text{GB}} \approx 1$ - 1+1 сервера для обеспечения ОЗУ мощности с резервом

Как итог для обепечения необходимых мощностей с учетом резерва было взято 5 серверов с вышеперечисленной конфигураций с приблизительной стоимостью за штуку 6000 USD.  
Общая стоимость развертывания - 30.000$  
Амортизация за 5 лет: 500$/месяц * 20 Дата-центров = 10.000$/месяц 
...
Для выдерживания пикового трафика необходима пропускная способность S3 - 66 TB/сек<br>
Базовая конфигурация S3 сервера CDN:
* Форм-фактор: 1U/2U
* CPU: 2×Intel Xeon Gold/Platinum (2x16 cores)
* RAM: 512 GB DDR5
* ROM: 24×NVMe SSD (24x15.36 TB)
* Network: 100 Gb/s
* Cost: ~5000$

Итого: <br>
${2929 \text{PB}}/{368.64 \text{TB}} = 8137 \text{ cnt}$  
${66 \text{TB}}/{100 \text{Gb}} = 5407 \text{ cnt}$  
$\text{max(8137, 5407)} \times 5000 \text{ dollars} = 40.680.555 \text{ dollars}$
